...
public class DielectricPdf implements FallbackPdf{
    ...
    public DielectricPdf(Vector in,Vector normal,double refractionIndex){
        ...
        var cos=Math.min(in.neg().dot(normal),1);
        var sin=Math.sqrt(1-cos*cos);
        var r=1-refractionIndex*refractionIndex;
        r*=r;
        direction=refractionIndex*sin>1||r+(1-r)*Math.pow(1-cos,5)>Randoms.nextDouble()?Vector.reflect(in,normal):Vector.refract(in,normal,refractionIndex);
    }
