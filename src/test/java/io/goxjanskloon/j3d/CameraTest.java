...
public class CameraTest{
    @Test public void balls(){
        ...
        world.add(new Sphere(new Vector(0,0,-5),50,Color.BLUE,0,new Lambertian()));
        Hittable light=new Quadrilateral(new Vector(-5,11,-5),new Vector(10,0,0),new Vector(0,0,10),Color.WHITE,12,new Light());
        world.add(light);
        final BvhTree bvhTree=new BvhTree(world);
        final Ray ray=new Ray(new Vector(0,0,-20),new Vector(0,0,300));
        final Camera camera=new Camera(new NormalView(bvhTree),light,ray,new Vector(0,1,0),new Vector(1,0,0),600,360,8,1,Color.BLACK,600);
    ...
    @Test public void cornellBox(){
        ...
        Hittable light=new Quadrilateral(new Vector(-5,10,-5),new Vector(10,0,0),new Vector(0,0,10),Color.WHITE,5,new Light());
        world.add(light);
        final BvhTree bvhTree=new BvhTree(world);
        final Ray ray=new Ray(new Vector(0,0,-20),new Vector(0,0,300));
        final Camera camera=new Camera(new NormalView(bvhTree),light,ray,new Vector(0,1,0),new Vector(1,0,0),600,360,8,1,Color.BLACK,30);
        final Image image=camera.render();
        if(image!=null){
            try{
